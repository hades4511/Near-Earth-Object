...
def populates_filters(**kwargs) -> tuple:
    ...
    for param_name, value in kwargs.items():
        ...
        filter_op = operator_options['default']
        class_type = DateFilter if param_name == 'date' else HazardFilter

        if len(split_param_name) > 1:
            operator_key = split_param_name[-1]
            class_key = split_param_name[0]

            if split_param_name[-1] == 'date':
                operator_key = split_param_name[0]
                class_key = 'date'

            filter_op = operator_options[operator_key]
            class_type = class_options[class_key]

        filters.append(class_type(filter_op, value))

    return tuple(filters)


def create_filters(
        date: datetime.date = None, start_date: datetime.date = None, end_date: datetime.date = None,
        distance_min: float = None, distance_max: float = None,
        velocity_min: float = None, velocity_max: float = None,
