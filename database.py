...
class NEODatabase:
    ...
    def fill_neos(self, neos: tuple):
        """Fill neo data in _neos and _named_neos.

        _neos will contain designation as keys and NearEarthObject object as values.
        _named_neos will contain names as key and designation as ID. This will prevent duplication of data.
        """
        for neo in neos:
            self._neos[neo.designation] = neo
            if neo.name:
                self._named_neos[neo.name] = neo.designation
    ...
    def get_neo_by_name(self, name: str) -> (NearEarthObject, None):
        ...
        """
        try:
            return self._neos[self._named_neos[name]]
        except KeyError:
            return None

    def query(self, filters: tuple = ()) -> iter:
        ...
        for approach in self._approaches:
            if all(param_filter(approach) for param_filter in filters):
                yield approach
